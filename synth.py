...
class RissetSnare(Shell):
  ...
  """
  def __init__(self, pitches, dur, dur_fact=1, noise_freq=500, randi_freq=500):
    Shell.__init__(self, dur, num=1, dur_fact=1)
    self.f1 = ExpTable(list=[(0, 0.0), (4, 1.0), (8192, 0.0)], exp=10)
    self.f2 = ExpTable(list=[(0, 0.0), (4, 1.0), (8192, 0.0)], exp=15)
    self.t1 = TrigEnv(Trig(), table=self.f1, dur=dur, mul=.09)
    self.t2 = TrigEnv(Trig(), table=self.f2, dur=dur, mul=.05)
    self.randi = Randi(freq=randi_freq, mul=self.t2)
    self.noise = Sine(freq=noise_freq, mul=self.randi)
    ...
    for i, pitch in enumerate(self.pitches):
      self.inharms[i] = Sine(freq=[pitch, pitch*1.6, pitch*2.2, pitch*2.3], mul=[.5, .75, 1., .75]) * .06 * self.t2 
  ...
  def out(self):
    voices = self.num if self.num > 1 else 2 
    return Mix([self.noise + self.inharms[i] + self.funds[i] for i in range(0, self.num)], mul=1./self.num, voices=voices).out()
...
#  p = Pitches(degrees=[0, 7, 10, 12], root=400)
  ...
  snr = RissetSnare(pitches=p, dur=5, noise_freq=400, randi_freq=500)
  mul_sig = Sig(.8)
  out = snr.out()
#   out.setMul(mul_sig)
  env = snr.getEnvs()
